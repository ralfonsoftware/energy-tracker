...
describe('useReactivateInsight', () => {
  beforeEach(() => {
    mockPatchInsight.mockReset()
  })

  it('useReactivateInsight_OnSuccess_InvalidatesInsightsQueryAndCallsPatchWithFalse', async () => {
    mockPatchInsight.mockResolvedValue({ insightId: 'i-1', isDismissed: false, dismissedAt: null, rowVersion: 'rv-2' })
    const { wrapper, invalidateQueries } = createWrapper()
    const { result } = renderHook(() => useReactivateInsight('flat-1'), { wrapper })
    ...
    expect(mockPatchInsight).toHaveBeenCalledWith('flat-1', 'i-1', false, 'rv-1')
  })

  it('useReactivateInsight_FlatIdUndefined_MutationRejects', async () => {
    const { wrapper } = createWrapper()
    const { result } = renderHook(() => useReactivateInsight(undefined), { wrapper })
