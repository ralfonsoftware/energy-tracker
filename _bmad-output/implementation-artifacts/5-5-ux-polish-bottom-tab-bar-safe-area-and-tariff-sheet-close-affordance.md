...
- [x] Task 6: Self-review pass before marking ready for review
  - [x] Manually verify (dev server, responsive device toolbar or a real iOS Safari session if available) that: the tab bar's icons remain visually at the same height as before on a normal viewport (no `env()` support), and grow only their bottom safe-area padding on a simulated notched viewport; dashboard/settings content is not clipped behind the bar on either; the tariff sheet's close button is visibly brighter/larger than before at rest (not just on hover). **Confirmed (2026-07-05, recorded during the Epic 5 retrospective):** Ralf performed this check on a real iOS/Safari session — verified working as expected. The check was actually done at the time of implementation; the checkbox/note here simply weren't updated until the retro surfaced the gap.
...
- Code review (3-layer parallel: Blind Hunter, Edge Case Hunter, Acceptance Auditor): fixed 4 confirmed test-correctness issues (wrong-element assertion in `TariffList` test, unisolated navigation assertion in `AppShell` test, scroll-reset effect missing `search`/`hash` deps, unrestored `vi.spyOn` on shared prototype); 8 items deferred as pre-existing/out-of-scope (see Review Findings and `deferred-work.md`); status set to `done` (Date: 2026-07-05)
- Task 6's manual real-device check (real iOS/Safari session) confirmed done during the Epic 5 retrospective — the check happened at implementation time but was left unrecorded in this file until now (Date: 2026-07-05)
...
- Task 6's automated checks (tsc, full vitest suite, lint) are green. Task 6's manual real-device/responsive-toolbar visual check was performed by Ralf on a real iOS/Safari session — confirmed working as expected (recorded 2026-07-05 during the Epic 5 retrospective; the check itself happened at implementation time but the story file wasn't updated until the gap surfaced in retro).
