...
echo ""
echo "4. Event Grid blob-trigger wiring is a TWO-PHASE deploy (Story 6.1):"
echo "   The Event Grid subscription's webhook URL needs a 'blobs_extension' system key"
echo "   that the Functions host only generates after ProcessImportFunction's code has"
echo "   been deployed and the host has started at least once."
echo "   a. First, deploy this story's application code via the normal CI/CD push to main"
echo "      so ProcessImportFunction goes live and the host starts at least once."
echo "   b. Only then re-run ./infra/deploy.sh (with the Event Grid resources already in"
echo "      main.bicep) to provision the Event Grid system topic/subscription."
echo "   Running step (b) before step (a) risks listKeys() returning a stale/empty"
echo "   blobs_extension value or the deployment failing outright."
